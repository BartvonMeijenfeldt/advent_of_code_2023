...
def get_answer() -> int:
    image = get_data()
    nr_energized_positions = get_max_nr_energized_positions(image)
...
def get_max_nr_energized_positions(image: list[list[str]]) -> int:
    return max(
        get_nr_energized_positions(image, start_direction, start_position)
        for i in range(len(image))
        for start_direction, start_position in [
            (R, (i, 0)),
            (L, (i, len(image[0]) - 1)),
            (D, (0, i)),
            (L, (len(image) - 1, i)),
...
def get_nr_energized_positions(
    image: list[list[str]], start_direction: tuple[int, int], start_pos: tuple[int, int]
) -> int:
    energized_positions = set()
    beam_positions_seen = set()

    beam_positions_to_parse = [(start_direction, start_pos)]
