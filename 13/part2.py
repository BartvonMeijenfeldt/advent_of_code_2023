...
def get_horizontal_reflection_below_column(image: list[str]) -> Optional[int]:
    for reflection_index in range(len(image) - 1):
        if is_horizontal_reflection_with_1_smudge(image, reflection_index):
            return reflection_index

    return None


def is_horizontal_reflection_with_1_smudge(image: list[str], reflection_index: int):
    nr_rows_one_off = 0
    i = 0
    while reflection_index - i >= 0 and reflection_index + i + 1 < len(image):
        nr_differences = get_nr_differences(image, reflection_index, i)
        if nr_differences > 1:
            return False

        if nr_differences == 1:
            nr_rows_one_off += 1
            if nr_rows_one_off > 1:
                return False

        i += 1

    return nr_rows_one_off == 1


def get_nr_differences(image: list[str], reflection_index: int, offset: int) -> int:
    top_index = reflection_index - offset
    top_row = image[top_index]

    bottom_index = reflection_index + offset + 1
    bottom_row = image[bottom_index]

    return sum(c1 != c2 for (c1, c2) in zip(top_row, bottom_row))
