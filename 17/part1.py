...
class DirectionCoordinate(Coordinate):
    ...
    def neighbours(self) -> list[DirectionCoordinate]:
        neighbours = [self + direction for direction in NEIGHBOUR_DIRECTIONS[self.direction]]
...
def get_shortest_path_cost(map: list[list[int]]) -> int:
    start_coordinate = DirectionCoordinate(x=0, y=0, direction=R, repeats=0)
    ...
    while queue:
        ...
        for neighbour_coordinate in coordinate.neighbours:
            if not neighbour_coordinate.is_valid(map):
                continue

            neighbour_cost = coordinate_cost + neighbour_coordinate.step_onto_cost(map)

            if neighbour_coordinate in costs:
                continue

            costs[neighbour_coordinate] = neighbour_cost
            shortest_future_cost = neighbour_coordinate.get_shortest_cost_to_goal(goal)
            heuristic_cost = neighbour_cost + shortest_future_cost
            queue.put(neighbour_coordinate, heuristic_cost)
